...
export default function Navbar() {
  ...
  return (
    <header className="w-full font-sans">
      ...
      {/* Search + nav + icons */}
      <div className="bg-white border-b border-gray-100 px-6 py-2.5 flex items-center gap-5">
        <div className="relative">
          <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
          <input
            type="text"
            placeholder="Search Product..."
            className="text-sm pl-8 pr-3 py-1.5 border border-gray-200 rounded-full w-44 bg-gray-50 focus:outline-none focus:border-[#c9a96e]"
          />
        </div>

        <nav className="flex items-center gap-6 flex-1 ml-2">
          {navLinks.map((link) => (
            <Link
              key={link.label}
              href={link.href}
              className={`relative text-xs font-medium tracking-wide ${
                link.highlight ? "text-[#c9a96e]" : "text-gray-500 hover:text-gray-900"
              }`}
            >
              {link.label}
              {link.badge && (
